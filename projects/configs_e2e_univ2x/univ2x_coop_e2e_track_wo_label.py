...
model_other_agent_inf = dict(
    ...
    loss_cfg=dict(
        ...
        assigner=dict(
            ...
            cls_cost=dict(type="FocalLossCost", weight=0.0),  # Disabled for wo_label
            reg_cost=dict(type="BBox3DL1Cost", weight=0.0),  # Disabled for wo_label
            pc_range=inf_point_cloud_range,
        ),
        loss_cls=dict(
            type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=0.0  # Disabled for wo_label
        ),
        loss_bbox=dict(type="L1Loss", loss_weight=0.0),  # Disabled for wo_label
    ...
    pts_bbox_head=dict(
        ...
        loss_bbox=dict(type="L1Loss", loss_weight=0.0),  # Disabled for wo_label
        loss_iou=dict(type="GIoULoss", loss_weight=0.0),  # Disabled for wo_label
    ),
...
data_root = "datasets/V2X-Seq-SPD-test-New/cooperative/"
info_root = "data/infos/V2X-Seq-SPD-test-New/cooperative/"
ann_file_train = info_root + f"spd_infos_temporal_train.pkl"
ann_file_val = info_root + f"spd_infos_temporal_test.pkl"
ann_file_test = info_root + f"spd_infos_temporal_test.pkl"

# Add command file for wo_label inference
command_file_test = info_root + "spd_infos_temporal_test_command.pkl"

split_datas_file = "./data/split_datas/cooperative-split-data-spd.json"
v2x_side = 'cooperative'
eval_mod = ['det', 'track']

# Modified training pipeline for wo_label - disable most annotations
train_pipeline = [
    ...
    dict(
        ...
        with_bbox_3d=False,  # Disabled for wo_label
        with_label_3d=False,  # Disabled for wo_label
        with_attr_label=False,

        with_future_anns=False,  # Disabled for wo_label
        with_ins_inds_3d=False,  # Disabled for wo_label
        ins_inds_add_1=True,  # ins_inds start from 1
    ),

    # Disabled for wo_label
    ...
    dict(type="DefaultFormatBundle3D", class_names=class_names, with_label=False),  # No labels
    dict(
        type="CustomCollect3D",
        keys=[
            # Minimal keys for wo_label training
            "veh2inf_rt",
            "img",
            "timestamp",
            "l2g_r_mat",
...
# Simplified test pipeline for wo_label
test_wo_label_pipeline = [
...
# Standard test pipeline (kept for compatibility)
test_pipeline = [
    dict(type='LoadMultiViewImageFromFilesInCeph', to_float32=True,
         file_client_args=file_client_args, img_root=data_root),
    dict(type="NormalizeMultiviewImage", **img_norm_cfg),
...
data = dict(
    ...
    train=dict(
        ...
        pipeline=train_pipeline,  # Uses modified wo_label pipeline
        classes=class_names,
        modality=input_modality,
        test_mode=False,
        use_valid_flag=False,  # Disabled for wo_label since no GT annotations
    ...
    val=dict(
        ...
        pipeline=test_wo_label_pipeline,  # Uses simplified wo_label pipeline
        patch_size=patch_size,
        canvas_size=canvas_size,
        bev_size=(bev_h_, bev_w_),
        predict_steps=predict_steps,
        ...
        class_range=class_range,
        other_agent_names=other_agent_names,
        inference_wo_label=True,  # Enable inference without labels
    ),
    test=dict(
        type=dataset_type,
        file_client_args=file_client_args,
        ...
        command_file=command_file_test,  # Add command file for wo_label inference
        pipeline=test_wo_label_pipeline,  # Uses simplified wo_label pipeline
        patch_size=patch_size,
        canvas_size=canvas_size,
        bev_size=(bev_h_, bev_w_),
...
total_epochs = 10
evaluation = dict(interval=1, pipeline=test_wo_label_pipeline)  # Use wo_label pipeline for evaluation
